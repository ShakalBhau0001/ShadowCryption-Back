...
from ..core.image_stego import (
    embed_payload_in_image_file,
    extract_payload_from_image_file,
)
from ..core.constants import IMAGE_MAGIC, HEADER_SIZE
...
def encode_image(
    image: UploadFile = File(...), password: str = Form(...), text: str = Form(...)
):
    ...
    inp = save_upload_file_tmp(image)
    out_tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".png")
    out_tmp.close()
    try:
        payload = encrypt_and_make_payload(text.encode("utf-8"), password, IMAGE_MAGIC)
        embed_payload_in_image_file(inp, payload, out_tmp.name)
...
def decode_image(image: UploadFile = File(...), password: str = Form(...)):
    if not password:
        raise HTTPException(status_code=400, detail="`password` required")

    inp = save_upload_file_tmp(image)
    try:
        header = extract_payload_from_image_file(inp, HEADER_SIZE)
        ...
        full_payload = extract_payload_from_image_file(inp, HEADER_SIZE + enc_len)
        _, encrypted_bytes = (
